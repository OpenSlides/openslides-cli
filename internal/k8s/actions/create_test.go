...
func TestWriteSecretFile(t *testing.T) {
	...
	secretPath := filepath.Join(tmpDir, "test_secret")
	secretValue := "my-secret-password"

	// Write secret
	err = writeSecretFile(secretPath, secretValue)
	if err != nil {
	...
	if string(content) != secretValue {
		t.Errorf("Secret content = %q, want %q", string(content), secretValue)
	}
}

func TestWriteSecretFile_NoNewline(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "create-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	// Verify no trailing newline (matching shell's printf behavior)
	if string(content) != secretValue {
		t.Errorf("Secret should not have trailing newline, got %q", string(content))
	}
}

func TestWriteSecretFile_Overwrite(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "create-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	if err := writeSecretFile(secretPath, "old-secret"); err != nil {
		t.Fatalf("First writeSecretFile failed: %v", err)
	}

	// Overwrite with new secret
	newSecret := "new-secret"
	if err := writeSecretFile(secretPath, newSecret); err != nil {
		t.Fatalf("Second writeSecretFile failed: %v", err)
	}
	...
	if string(content) != newSecret {
		t.Errorf("Secret content = %q, want %q", string(content), newSecret)
	}
}
